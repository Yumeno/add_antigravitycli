[CmdletBinding()]
param([string]$DestinationRoot = (Join-Path $env:USERPROFILE ".claude"))
$ErrorActionPreference = "Stop"
$Source = Join-Path (Split-Path $PSScriptRoot -Parent) ".claude\skills"
...
try {
 ...
 foreach($name in $Names){$final=Join-Path $SkillsRoot $name;$new="$final.new";$old="$final.old";Remove-Item -LiteralPath $new -Recurse -Force -ErrorAction SilentlyContinue;try{Move-Item -LiteralPath (Join-Path $Stage "skills\$name") -Destination $new -ErrorAction Stop}catch{throw "Failed to stage new skill: $name"};if(Test-Path -LiteralPath $final){Remove-Item -LiteralPath $old -Recurse -Force -ErrorAction SilentlyContinue;try{Move-Item -LiteralPath $final -Destination $old -ErrorAction Stop}catch{throw "Failed to retire current skill: $name"}};try{Move-Item -LiteralPath $new -Destination $final -ErrorAction Stop}catch{if(Test-Path -LiteralPath $old){try{Move-Item -LiteralPath $old -Destination $final -ErrorAction Stop}catch{[Console]::Error.WriteLine("Rollback also failed for ${name} (leftover: $old): $_")}};throw "Failed to promote new skill: $name"};Remove-Item -LiteralPath $old -Recurse -Force -ErrorAction SilentlyContinue}
