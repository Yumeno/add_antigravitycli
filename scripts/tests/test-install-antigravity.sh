#!/usr/bin/env bash
set -u
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)"; INSTALL="$HERE/../install-for-antigravity.sh"; ROOT="$(mktemp -d "${TMPDIR:-/tmp}/add_antigravitycli_install.XXXXXX")"; DEST="$ROOT/install root"; trap 'chmod -R u+w "$ROOT" 2>/dev/null || true;rm -rf "$ROOT"' EXIT
