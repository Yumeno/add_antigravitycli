...
try {
    ...
    Test-Case "read-only skills directory preserves prior content on failure" {
        ...
        $acl = Get-Acl -LiteralPath $skills; $identity = [Security.Principal.WindowsIdentity]::GetCurrent().User
        ...
        finally { $restoredAcl = Get-Acl -LiteralPath $skills; [void]$restoredAcl.RemoveAccessRule($rule); Set-Acl -LiteralPath $skills -AclObject $restoredAcl -ErrorAction Stop }
    }
