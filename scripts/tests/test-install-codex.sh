#!/usr/bin/env bash
set -u
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)"; INSTALL="$HERE/../install-for-codex.sh"; SOURCE="$HERE/../../.agents/skills"
ROOT="$(mktemp -d "${TMPDIR:-/tmp}/add_antigravitycli_install.XXXXXX")"; DEST="$ROOT/install root"; trap 'chmod -R u+w "$ROOT" 2>/dev/null || true; rm -rf "$ROOT"' EXIT
...
t_stale_old(){ run && mv "$DEST/skills/ask-antigravity" "$DEST/skills/ask-antigravity.old" && run && [[ -f "$DEST/skills/ask-antigravity/SKILL.md" && ! -e "$DEST/skills/ask-antigravity.old" && ! -e "$DEST/skills/ask-antigravity.new" ]]; }
...
check installs_skills t_install;check no_legacy_placeholders t_placeholders;check bundled_helpers t_helpers;check preserves_unrelated_and_replaces_stale t_preserve;check idempotent_and_absorbs_new t_idempotent;check absorbs_stale_old_crash_recovery t_stale_old;check readonly_preserves_prior_content t_readonly;check cleans_artifacts t_clean
