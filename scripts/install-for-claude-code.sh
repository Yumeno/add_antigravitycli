#!/usr/bin/env bash
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)"; SOURCE="$HERE/../.claude/skills"
...
for name in "${names[@]}"; do
 final_dest="$destination_root/skills/$name"; new_dest="$final_dest.new"; old_dest="$final_dest.old"; rm -rf -- "$new_dest" "$old_dest"; mv -- "$stage/skills/$name" "$new_dest"
