...
- **保存先はリポジトリ内の絶対パスで仕様書に明示する**。相対・曖昧な指定はagyのscratchディレクトリに落ち、Git検収に映らない
- 仕様書にはファイル名・形式・被写体・スタイル・避けたい要素を書く。詳細なプロンプト構築はagentが自動で行う
- 編集はリファレンス画像の絶対パスを仕様書に書くだけでよい(`--attachment` 不要)。「変更点の限定列挙 + 維持項目の明示列挙」で指示する
