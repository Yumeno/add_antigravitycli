...
- **agy 1.2.7 の headless + sandbox では agent のシェルコマンドが権限拒否される(実測範囲)**。生成物を指定パスへ複製・変換させると空応答になる。仕様書では「複製・変換・コマンド実行をしない」「`ARTIFACT_PATH:` に生成物の絶対パスを報告する」と指示する(1.2.7 で 1 件確認)。**helper の自動検収は生成物を見ない**ので、helper 終了後に host が同梱の `antigravity-artifact.ps1 import -Repo <repo> -Source <ARTIFACT_PATH> -Destination <repo 内パス>`(bash は `antigravity-artifact.sh import --repo ... --source ... --destination ...`)で取り込む。helper が source の実体パス(`~/.gemini/antigravity-cli/brain/` 配下、link 拒否)、magic bytes、寸法、拡張子の一致、保護対象、上書き(`-Overwrite` 必須)を検査し SHA-256 付きで `[ANTIGRAVITY_ARTIFACT_OK]` を出す。PNG が必要なら取り込み後に host が変換する。目視は省かない
